    ...
    public partial class CP_Supplier : Form
    {
        ...
        public void udfntphide()
        {
            try
            {
                ...
                tpBankName.ShowAlways = false;
                tpBankShortName.ShowAlways = false;
                tpBranchName.ShowAlways = false;
                tpAccountNo.ShowAlways = false;
                tpIfsCode.ShowAlways = false;
            }
            catch (Exception ex)
            {
                objError = new DataError();
        ...
        public void udfnMappingGridsLoad()
        {
            try
            {
                ...
                objDs = objspservice.udfnproductmasterlist(3, 0, 0,Convert.ToInt32(cmbMappingGroup.SelectedValue), Convert.ToInt32(cmbMappingSubGroup.SelectedValue),"", MainForm.pbUserID, MainForm.pbIpAddress, 0, Convert.ToInt32(cmbMappingorderschedule.SelectedValue), Convert.ToInt32(cmbMappingordeDay.SelectedValue));
                if (objDs.Tables[0].Rows.Count != 0)
                {
                    for (int i = 0; i < objDs.Tables[0].Rows.Count; i++)
                    {
                ...
                if (e.RowIndex != -1)
                    {
                        ...
                        case "clmedit":
                            ...
                            if (pbSupplierid != "")
                            {
                                ...
                                DataSet objDS;
                                objDS = objspservice.udfnSupplierList(3, Convert.ToInt32(pbSupplierid), Convert.ToInt32(grdSupplierList.SelectedRows[0].Cells["ID"].Value.ToString()), 0,0, "");
                                objspservice.CloseConnection();
                                if (objDS != null)
                                {
                                    if (objDS.Tables[0].Rows.Count > 0)
