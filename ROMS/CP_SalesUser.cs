...
{  //Created By:-Sathish
    ...
    public partial class CP_SalesUser : Form
    {
        ...
        public void udfnSave(object sender, EventArgs e)
        {
            try
            {
                ...
                if (varvalue[0] == "3")
                {
                    ...
                    MainForm.objCp_SalesUserList.udfnList();
        ...
        private void CP_SalesUser_Load(object sender, EventArgs e)
        {
            try
            {
            ...
            {
                MainForm.objCp_SalesUserList.picLoader.Visible = false;
                MainForm.objCp_SalesUserList.picLoader.SendToBack();
            }
        }
        ...
        private void udfnEdit()
        {
            try
            {
                if (varUserID != "")
                {
                    ...
                    DataSet objDs = new DataSet();
                    objDs = objspdservice.udfnSalesUserList(1, "", "", "", Convert.ToInt32(varUserID), 0, "");
                    ...
                    if (objDs != null)
                    {
                        if (objDs.Tables[0].Rows.Count > 0)
                        {
                            txtUserName.Text = objDs.Tables[0].Rows[0]["SU_Name"].ToString().Replace("''", "'");
                            oldUsername = objDs.Tables[0].Rows[0]["SU_LoginID"].ToString().Replace("''", "'");
                            txtLoginID.Text = objDs.Tables[0].Rows[0]["SU_LoginID"].ToString();
                            txtPassword.Text = objDs.Tables[0].Rows[0]["SU_Password"].ToString();
                            oldpassword = objDs.Tables[0].Rows[0]["SU_Password"].ToString();
                            txtCPassword.Text = objDs.Tables[0].Rows[0]["SU_Password"].ToString();
                            //  cmbUserRole.SelectedValue= objDS.Tables[0].Rows[0]["UserRoleCode"].ToString();
                            if (objDs.Tables[0].Rows[0]["SU_STSID"].ToString() == "1")
                            {
                                rbActive.Checked = true;
                            }
                            else
