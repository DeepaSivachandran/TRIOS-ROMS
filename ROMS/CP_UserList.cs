            ...
            if (privilege.Contains("4") || Convert.ToInt32(MainForm.pbUserRoleId) == 1)
            {
                try
                {
                    if (grdUserList.SelectedRows.Count > 0)
                    {
                        ...
                        {
                            SPDataService objspservice = new SPDataService();
                            varResult = objspservice.udfnUser(2, Convert.ToInt32(grdUserList.SelectedRows[0].Cells["ID"].Value.ToString()), "", "", 0, 0, "", 0, 0, "", "User Delete", varUserID, 0, null, 0, 0);
                            objspservice.CloseConnection();
                            if (varResult.Split('~')[0] == "3")
                            {
                                if (varResult.Split('~')[1] == "1")
                                {
                                    ...
                                    if (MainForm.objCP_Verify.flag == 1)
                                    {
                                        ...
                                        varResult = objspservice.udfnUser(2, Convert.ToInt32(grdUserList.SelectedRows[0].Cells["ID"].Value.ToString()), "", "", 0, 0, "", 0, 0, "", "User Delete", varUserID, 1, null, 0, 0);
                                        objspservice.CloseConnection();
                                        if (varResult.Split('~')[0] == "3")
                                        {
