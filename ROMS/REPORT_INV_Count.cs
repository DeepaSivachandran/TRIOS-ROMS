...
namespace ROMS
{
    public partial class REPORT_INV_Count : Form
    {
        ...
        public void udfnCountReport(int varFlag)
        {
            try
            {
                ...
                objTRNG_GoodsInward.ViewType = 6;
                ...
                objTRNG_GoodsInward.paraSLID = varLocationId;
                if (Convert.ToInt32(cmbReportType.SelectedValue) == 377)
                {
                    objTRNG_GoodsInward.paraFlag = 2;
                }
                else if ((Convert.ToInt32(cmbReportType.SelectedValue) == 378))
                {
                ...
                objDs = objspservice.udfnInwardList(objTRNG_GoodsInward);
                objspservice.CloseConnection();
                if (objDs != null) { if (objDs.Tables.Count > 0) { if (objDs.Tables[0].Rows.Count > 0) { varPrint = 1; } } }
                string varReportName = "", varHeaderName = "";
                if (varPrint == 1)
                {
                    ...
                    objBillreport.Load(Application.StartupPath + "\\Reports\\RPT_TP_Count.rpt");
                    if (Convert.ToInt32(cmbReportType.SelectedValue) == 377  )
                    {
                        objBillreport.SetParameterValue("paraFlag", 2);

                        //objBillreport.SetParameterValue("paraMonthsName", varMonthsName);
                        varReportName = "Outward_Count";
                        varHeaderName = "Outward Count";
                    }
                    else if ((Convert.ToInt32(cmbReportType.SelectedValue) == 378) )
                    {
                        objBillreport.SetParameterValue("paraFlag", 1); 
                        varReportName = "Inward_Count";
                        varHeaderName = "Inward Count";

                    }


                    objBillreport.SetParameterValue("paraCompanyCode", Convert.ToInt32(cmbConcern.SelectedValue));
                    objBillreport.SetParameterValue("paraReportName", varHeaderName);   
                    objBillreport.SetParameterValue("paraSlid", varLocationId);   
                    objBillreport.SetParameterValue("paraFromDate", dpFromDate.Text);   

                    objBillreport.SetParameterValue("paraComname", cmbConcern.Text);   
                    objBillreport.SetParameterValue("paraDateValue", dpFromDate.Text);   
                    objBillreport.SetParameterValue("paraLocationName", varLocationName);   
                    objBillreport.SetParameterValue("paraTypeValue", cmbReportType.Text);   
        ...
        private void REPORT_GRNSummary_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //Transaction id 	91
                objDataBind.BindComboBoxListSelected("DEF_MASTER", "MST_TransactionID IN (0,116) AND MSTID<>0     ORDER BY MST_OrderID ASC", "MST_DisplayText,MSTID,MST_ShortName", cmbReportType, "", "MST_DisplayText", "MSTID"); 
