    ...
    public partial class CP_RepresentativeList : Form
    {
                ...
                try
                {
                    if (grdreplist.SelectedRows.Count > 0)
                    {
                        ...
                        if (dialogResult == DialogResult.Yes)
                        {
                            ...
                            varResult = objspdservice.udfnRepMaster(2, Convert.ToInt32(grdreplist.SelectedRows[0].Cells["ID"].Value.ToString()), "", "", "", "", "", 0, "Representative Delete", varUserID, 0);
                            ...
                            if (varvalue[0] == "3")
                            {
                                if (varResult.Split('~')[1] == "1")
                                {
                                    ...
                                    if (MainForm.objCP_Verify.flag == 1)
                                    {
                                        ...
                                        varResult = objspdservice.udfnRepMaster(2, Convert.ToInt32(grdreplist.SelectedRows[0].Cells["ID"].Value.ToString()), "", "", "", "", "", 0, "Representative Delete", varUserID, 1);
        ...
        public void udfnlist()
        {
            try
            {
                ...
                objDs = objdserv.udfnRepMasterList(0,0, MainForm.pbUserID, MainForm.pbIpAddress,Convert.ToInt32(cmbStatus.SelectedValue));
                objdserv.CloseConnection();
                if (objDs != null)
                {
        ...
        public void udfnDefaultSearchGrid()
        {
            try
            {
                ...
                DGV_SearchGrid.Columns["BRANDID"].Visible = false; DGV_SearchGrid.ScrollBars = ScrollBars.Both;
            }
            catch (Exception ex)
            {
                objError = new DataError();
        ...
        private void Grdreplist_Scroll(object sender, ScrollEventArgs e)
        {
            try
            {
                if (lblNoRecordsFound.Visible == false)
                {
                    int totalWidth = 0;
                    int offSetValue = grdreplist.HorizontalScrollingOffset;
                    foreach (DataGridViewColumn col in DGV_SearchGrid.Columns)
                        totalWidth += col.Width;
                    if (totalWidth - grdreplist.Width > grdreplist.HorizontalScrollingOffset && grdreplist.HorizontalScrollingOffset > 0)
                    {
                        offSetValue = offSetValue;
                    }
                    DGV_SearchGrid.HorizontalScrollingOffset = offSetValue;
                    DGV_SearchGrid.Invalidate();
                    udfnscrollVisible(DGV_SearchGrid, grdreplist);
                }
            }
            catch (Exception ex)
            {
        ...
        private void BtnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if ((grdreplist.Rows.Count > 0))
                {
                    ...
                    ExcelSheet.Name = "Representative List";
                    int cIndex = 0;
                    int count = 0;
                    foreach (DataGridViewColumn col in grdreplist.Columns)
                    {
