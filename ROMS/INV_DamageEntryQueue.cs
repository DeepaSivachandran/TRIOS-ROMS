...
namespace ROMS
{
    public partial class INV_DamageEntryQueue : Form
    {
        ...
        public void udfnEdit(int varEditflag)
        {
            if (EditAccess == true || Convert.ToInt32(MainForm.pbUserRoleId) == 1)
            {
                try
                {
                    ...
                    //MainForm.objINV_DamageEntry.grdDamageEntry.Columns["clmremove"].Visible = false;
                    MainForm.objINV_DamageEntry.fromQueueFlag = 1;    
                    MainForm.objINV_DamageEntry.varTotalProd = Convert.ToInt32(grdInwardQueueList.SelectedRows[0].Cells["Total Products"].Value);
                    MainForm.objINV_DamageEntry.varQueid = Convert.ToInt32(grdInwardQueueList.SelectedRows[0].Cells["GOID"].Value);
                    MainForm.objINV_DamageEntry.varcomID = Convert.ToInt32(grdInwardQueueList.SelectedRows[0].Cells["COMID"].Value);
            ...
            try
            {
                if (varUpDownKeyLocation == 0)
                {
                    ...
                    if (txtStockLocation.Text.Length > 0)
                    {
                        ...
                        objMR_Location.paraId = 8;
                        objMR_Location.paraLocationName = txtStockLocation.Text.Trim();
                        objMR_Location.ParaCompanycode = Convert.ToInt32(0);
        ...
        public void udfnLvStockLocation()
        {
            try
            {
                ...
                {
                    varStockLocaionId = Convert.ToString(DGV_FilterLocation.SelectedRows[0].Cells["SLID"].Value.ToString());
        ...
        public void udfnList()
        {
            try
            {
                ...
                objDs = objspservice.udfnproductDamage(9, 0, 0, 0, 0, 0, "","", "", Convert.ToInt32(varPRID), varStockLocaionId, 0,0,0);
                ...
                if (objDs != null)
                {
                    if (objDs.Tables.Count != 0)
                    {
                        ...
                        if (objDs.Tables[0].Rows.Count != 0)
                        {
                            ...
                            grdInwardQueueList.Columns["Outward Date"].Width = 120;
                            grdInwardQueueList.Columns["Outward No."].Width = 120;
                            grdInwardQueueList.Columns["Stock Location"].Width = 150;
                            grdInwardQueueList.Columns["Transaction Type"].Visible = false;
                            grdInwardQueueList.Columns["GOID"].Visible = false;
                            grdInwardQueueList.Columns["COMID"].Visible = false;
                            grdInwardQueueList.Columns["Total Products"].Width = 120;
                            grdInwardQueueList.Columns["Created By"].Width = 120;
                            grdInwardQueueList.Columns["Created On"].Width = 150;
                            grdInwardQueueList.Columns["STSID"].Visible = false;
                            grdInwardQueueList.Columns["Status"].Visible = false;
                            ...
                            grdInwardQueueList.Columns["Outward Date"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
                        }
                        else
                        {
        ...
        private void TsbInwardList_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                MainForm.objINV_DamageEntryList.MdiParent = this.ParentForm;
