...
namespace ROMS
{
    public partial class CP_CostPrice_Update_Bulk_Approval_List : Form
    {
        ...
        public CP_CostPrice_Update_Bulk_Approval_List()
        {
            InitializeComponent();
                ...
                if (objDs != null)
                {
                    if (objDs.Tables.Count != 0)
                    {
                        ...
                        if (objDs.Tables[0].Rows.Count != 0)
                        {
                            ...
                            grdCPApproval.DataSource = objDs.Tables[0];
                            grdCPApproval.Columns["S.No."].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                            ...
                            grdCPApproval.Columns["ID"].Visible = false;
                            grdCPApproval.Columns["Status ID"].Visible = false; 
                        }
                        else
                        {
        ...
        private void udfnSearchGridHead()
        {
            try
            {
                ...
                {
                    udfnGridSearchHeading(grdCPApproval, DGV_SearchGrid);
                    DGV_SearchGrid.Columns.Clear();
                    List<int> visibleColumns = new List<int>();
                    foreach (DataGridViewColumn col in grdCPApproval.Columns)
                    {
        ...
        public void udfndelete()
        {
            if (privilege.Contains("4") || Convert.ToInt32(MainForm.pbUserRoleId) == 1)
            {
                try
                {
                    if (grdCPApproval.SelectedRows.Count > 0)
                    {
                        ...
                        if (dialogResult == DialogResult.Yes)
                        {
                            ...
                            objRateChange.paraStatusId = Convert.ToInt16(grdCPApproval.SelectedRows[0].Cells["Status ID"].Value.ToString());

                            objRateChange.paraProductID = Convert.ToInt16(grdCPApproval.SelectedRows[0].Cells["ID"].Value.ToString());  
            ...
            if (privilege.Contains("3") || Convert.ToInt32(MainForm.pbUserRoleId) == 1)
            {
                try
                {
                    if (grdCPApproval.SelectedRows.Count != 0)
                    {
                        ...
                        MainForm.objCP_CostPrice_Update_Bulk.varId = Convert.ToInt32(grdCPApproval.SelectedRows[0].Cells["ID"].Value);
                        MainForm.objCP_CostPrice_Update_Bulk.grid_flag = Convert.ToInt32(grdCPApproval.SelectedRows[0].Cells["Status ID"].Value);
                        MainForm.objCP_CostPrice_Update_Bulk.varViewType = 2;
                        //objMainForm.CenterEntryForm(this, MainForm.objINV_StockJournalConversion);
                        MainForm main = (MainForm)this.MdiParent;
                        main.IsEntryFormOpen = true;
                        main.CurrentEntryForm = MainForm.objCP_CostPrice_Update_Bulk;
        ...
        private void GrdSubGroupList_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            try
            {
                for (int i = 0; i < grdCPApproval.Rows.Count; i++)
                {
                    if (Convert.ToString(grdCPApproval.Rows[i].Cells["Status ID"].Value) == "122")
                    {
                        grdCPApproval.Rows[i].Cells["Status"].Style.BackColor = Color.LimeGreen;
                        grdCPApproval.Rows[i].Cells["Status"].Style.ForeColor = Color.White;
                    }
                    else
                    {
                        grdCPApproval.Rows[i].Cells["Status"].Style.BackColor = Color.Tomato;
                        grdCPApproval.Rows[i].Cells["Status"].Style.ForeColor = Color.White;
                    }
                }
            }
                ...
                if ((grdCPApproval.Rows.Count > 0))
                {
                    ...
                    foreach (DataGridViewColumn col in grdCPApproval.Columns)
                    {
                        if (col.Visible)
                        {
                            ...
                            int varSLno = 1;
                            foreach (DataGridViewRow rowa in grdCPApproval.Rows)
                            {
                                if (cIndex == 1)
                                {
        ...
        private void DGV_SearchGrid_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                DataService objDser = new DataService();
                grdCPApproval.DataSource = objDser.udfnGridSearchFilter(DGV_SearchGrid, grdCPApproval);
                objDser.CloseConnection();
                grdCPApproval.HorizontalScrollingOffset = DGV_SearchGrid.HorizontalScrollingOffset;
        ...
        private void DGV_SearchGrid_ColumnHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (lblNoRecordsFound.Visible == false)
            {
                DataGridViewColumn newColumn = grdCPApproval.Columns[e.ColumnIndex];
                DataGridViewColumn oldColumn = grdCPApproval.SortedColumn;
                ...
                if (oldColumn != null)
                {
                    ...
                    if (oldColumn == newColumn &&
                        grdCPApproval.SortOrder == SortOrder.Ascending)
                    {
                ...
                grdCPApproval.Sort(newColumn, direction);
                newColumn.HeaderCell.SortGlyphDirection =
                    direction == ListSortDirection.Ascending ?
                    SortOrder.Ascending : SortOrder.Descending;
                DataGridViewColumn DGV = DGV_SearchGrid.Columns[e.ColumnIndex];
                DGV.HeaderCell.SortGlyphDirection = SortOrder.None;
                DGV_SearchGrid.HorizontalScrollingOffset = grdCPApproval.HorizontalScrollingOffset;
                DGV_SearchGrid.FirstDisplayedScrollingRowIndex = 0;
            }
        }
        ...
        private void DGV_SearchGrid_ColumnWidthChanged(object sender, DataGridViewColumnEventArgs e)
        {
            try
            {
                if (grdCPApproval.ColumnCount > 0)
                {
                    grdCPApproval.Columns[e.Column.Index].Width = e.Column.Width;
                    DGV_SearchGrid.HorizontalScrollingOffset = grdCPApproval.HorizontalScrollingOffset;
                }
            }
            catch (Exception ex)
        ...
        private void DGV_SearchGrid_Scroll(object sender, ScrollEventArgs e)
        {
            try
            {
                ...
                {
                    int totalWidth = 0;
                    int offSetValue = grdCPApproval.HorizontalScrollingOffset;
                    foreach (DataGridViewColumn col in DGV_SearchGrid.Columns)
                        totalWidth += col.Width;
                    if (totalWidth - grdCPApproval.Width > grdCPApproval.HorizontalScrollingOffset && grdCPApproval.HorizontalScrollingOffset > 0)
                    {
        ...
        private void GrdSubGroupList_Scroll(object sender, ScrollEventArgs e)
        {
            try
            {
                if (lblNoRecordsFound.Visible == false)
                {
                    ...
                    DGV_SearchGrid.HorizontalScrollingOffset = offSetValue;
                    DGV_SearchGrid.Invalidate();
                    udfnscrollVisible(DGV_SearchGrid, grdCPApproval);
                }
            }
            catch (Exception ex)
            {
