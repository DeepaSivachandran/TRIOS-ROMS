    ...
    public partial class REPORT_CP_HSN : Form
    {
        ...
        public void udfnHSNProduct()
        {
            try
            {
                ...
                DataSet objDs = new DataSet();
                SPDataService objspservice = new SPDataService();
                objDs = objspservice.udfnproductmasterlist(16, 0, 0,0,0,"","","",0, Convert.ToInt32(cmbStatus.SelectedValue), 0,0,0,0, Convert.ToInt32(cmbHSN.SelectedValue), Convert.ToInt32(cmbGST.SelectedValue),0,0,0,0,0,"",0,"");
                objspservice.CloseConnection();
                if (objDs != null) { if (objDs.Tables.Count > 0) { if (objDs.Tables[0].Rows.Count > 0) { varPrint = 1; } } }
                if (varPrint == 1)
                {
