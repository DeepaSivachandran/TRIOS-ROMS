...
namespace ROMS
{
    public partial class PAY_SupplierPayment : Form
    {
        ...
        public int clearClick = 0, varApplyFlag = 0, varPaymentStatus = 0, varCreatemodeFlag = 0;
        ...
        public void udfnSubtotalCalc()
        {
            try
            {
                ...
                for (int i= 0; i < grdSupplierPayment.Rows.Count;i++)
                {
                    if (Convert.ToString(grdSupplierPayment.Rows[i].Cells["clmInvoiceAmnt"].Value)!="")
                    {
                        ...
                        //varAdvanceAmnt = Convert.ToDecimal(grdSupplierPayment.Rows[i].Cells["clmAdvanceAmnt"].Value);
            ...
            {
                lblSubtotal.Text = varSubtotal.ToString("#,##0.00");
                    ...
                    if (grdSupplierPayment.Columns[e.ColumnIndex].Name == "clmcheck")
                    {
                        ...
                        else
                        {
                            ...
                            for (int i = 0; i < dtAdvance.Rows.Count; i++)
                            {
                                ...
                                {
                                    //varCurrentBalance = Convert.ToDecimal(dtAdvance.Rows[i]["current Balance"]);
                            ...
                            { 
                                for (int i = 0; i < dtAdvance.Rows.Count; i++)
                                {
                                    ...
                                    if (varMaxSNo == varSNO)
                                    {
                                        dtAdvance.Rows[i]["Current Balance"] = varBalance + Convert.ToDecimal(grdSupplierPayment.Rows[e.RowIndex].Cells["clmAdvanceAmnt"].Value);
                                    }
                                }
                            }
